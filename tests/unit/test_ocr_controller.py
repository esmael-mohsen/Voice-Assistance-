...
def test_ocr_process_controller_starts_and_stops_subprocess(tmp_path: Path) -> None:
    ...
    start_request = build_request(capability_id="ocr", action="start", timeout_seconds=0.1)
    ...
    stop_request = build_request(capability_id="ocr", action="stop", timeout_seconds=0.1)
...
def test_ocr_process_controller_reports_already_running(tmp_path: Path) -> None:
    project_dir = tmp_path / "ocr-project"
    project_dir.mkdir()
    (project_dir / "main.py").write_text("print('ocr app')\n", encoding="utf-8")
    python_path = project_dir / ".venv" / "Scripts" / "python.exe"
    ...
    request = build_request(capability_id="ocr", action="start", timeout_seconds=0.1)
...
def test_ocr_process_controller_prefers_env_python_then_pi_venv(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_dir = tmp_path / "ocr-project"
    ...
    monkeypatch.setenv("EGB_OCR_PYTHON", str(env_python))
...
def test_ocr_process_controller_uses_pi_venv_before_windows_venv(tmp_path: Path) -> None:
    project_dir = tmp_path / "ocr-project"
    project_dir.mkdir()
    (project_dir / "main.py").write_text("print('ocr app')\n", encoding="utf-8")
...
def test_ocr_process_controller_uses_configured_terminal_command(monkeypatch, tmp_path: Path) -> None:
    project_dir = tmp_path / "ocr-project"
    project_dir.mkdir()
    (project_dir / "main.py").write_text("print('ocr app')\n", encoding="utf-8")
...
def test_ocr_process_controller_maps_shared_camera_index_to_assistant_index(monkeypatch, tmp_path: Path) -> None:
    project_dir = tmp_path / "ocr-project"
    project_dir.mkdir()
    (project_dir / "main.py").write_text("print('ocr app')\n", encoding="utf-8")
    monkeypatch.setenv("EGB_OCR_COMMAND", "bash -lc 'source .venv/bin/activate && exec python -u main.py'")
    ...
    assert result.status == "success"
    assert launches[0]["env"]["ASSISTANT_CAMERA_INDEX"] == "2"
    assert "ASSISTANT_CAMERA_SOURCE" not in launches[0]["env"]
